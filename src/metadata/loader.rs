...
use crate::error;
use crate::error::Metadata::MismatchedTag;
...
fn metadata<R: BufRead>(reader: &mut Reader<R>) -> Result<Vec<Metadata>, error::LoadMetadata> {
    ...
    loop {
        match reader.read_event_into(&mut buffer)? {
            ...
            Event::End(ref e) if e.name().as_ref() != b"phoneNumberMetadata" => {
                return Err(error::Metadata::MismatchedTag(
                    str::from_utf8(e.name().as_ref())?.into(),
                )
                .into())
            }

            Event::End(ref e) if e.name().as_ref() == b"phoneNumberMetadata" => return Ok(result),
...
fn territories<R: BufRead>(reader: &mut Reader<R>) -> Result<Vec<Metadata>, error::LoadMetadata> {
    ...
    loop {
        match reader.read_event_into(&mut buffer)? {
            ...
            Event::End(ref e) if e.name().as_ref() == b"territories" => return Ok(result),
...
fn territory<'a, R: BufRead>(
    ...
    for attr in e.attributes() {
        let Attribute { key, value } =
            attr.map_err(|e| error::LoadMetadata::Metadata(MismatchedTag(e.to_string())))?;
    ...
    loop {
        match reader.read_event_into(&mut buffer)? {
            ...
            Event::End(ref e) if e.name().as_ref() == b"territory" => return Ok(meta),
    ...
    loop {
        match reader.read_event_into(&mut buffer)? {
            ...
            Event::Empty(ref e) => match e.name().as_ref() {
                b"possibleLengths" => {
                    for attr in e.attributes() {
                        let Attribute { key, value } = attr.map_err(|e| {
                            error::LoadMetadata::Metadata(MismatchedTag(e.to_string()))
                        })?;
            ...
            Event::End(ref e) if e.name().as_ref() == name => return Ok(descriptor),
...
fn formats<R: BufRead>(
    ...
    loop {
        match reader.read_event_into(&mut buffer)? {
            ...
            Event::End(ref e) if e.name().as_ref() == name => return Ok((national, international)),
...
fn ignore<R: BufRead>(reader: &mut Reader<R>, name: &[u8]) -> Result<(), error::LoadMetadata> {
    ...
    loop {
        match reader.read_event_into(&mut buffer)? {
            ...
            Event::End(ref e) if e.name().as_ref() == name => return Ok(()),
...
fn text<R: BufRead>(reader: &mut Reader<R>, name: &[u8]) -> Result<String, error::LoadMetadata> {
    ...
    loop {
        match reader.read_event_into(&mut buffer)? {
            ...
            Event::End(ref e) if e.name().as_ref() == name => return Ok(result),
